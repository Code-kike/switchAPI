...
- [ ] Web SPA：登录、供应商管理（含预设/备选序列拖排）、一键切换、用量仪表盘/趋势/明细、
  设备管理、事件时间线；ws/ui 实时刷新
- [ ] Hub：embed 前端产物；ws/ui 通道
- [ ] 桌面壳：Tauri 装载 + 首启向导（Hub 地址/登录）+ 托盘快切 + 开机自启 + Agent 托管 + 应急页
- [ ] Docker 镜像 + hub 部署文档；桌面打包（三平台）
- 验证：手机浏览器与桌面壳同时打开，任一端切换/新用量在另一端 1 秒内可见
